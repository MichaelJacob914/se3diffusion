...
class r3_diffuser: 
 # ──────────────────────────────────────────────────────────────────────
    ...
    def _eu_sample(self, x_t, t, noise,
        ...
        alpha_bar_t  = self.alpha_bars[t]            # \bar α_t
        ...
        if t > 1:
            sigma_t = torch.sqrt(beta_t)             # or use the true posterior σ̃_t
            x_tm1 = mu + sigma_t * torch.randn_like(x_t)
        else:
            # t = 1 → deterministic final step
            x_tm1 = mu

        # --- optional guidance step ------------------------------------------
        if guidance and t % optim_steps == 0:
            # e.g. DPS / ILVR / classifier guidance
    ...
    def _eu_sample_batch(
        ...
        if t > 1:
            ...
            #v_noise = np.sqrt(self.betas[t]) * torch.randn_like(x_t)
        else:
            v_noise = torch.zeros_like(x_t)


        v_tensor = (1 - self.alpha_bars[t])/(torch.sqrt(1 - self.alpha_bars[t])) *  eps_pred
        #x_prev = torch.sqrt(self.alpha_bars[t-1]) * x_t + torch.sqrt(1 - self.alpha_bars[t-1]) * eps_pred
        x_prev = (1 / torch.sqrt(torch.tensor(self.alphas[t]))) * (x_t - v_tensor) + v_noise

        # --- optional guidance -------------------------------------------------
        if guidance and t % optim_steps == 0:
            with torch.no_grad():                         # safe detaching inside
                x_prev = self.descent(x_prev,
                                    # x₀ reconstruction (only if descent needs it)
